...
import { CreateFunctionType, UpdateFunctionType } from "@/common/schema/function";
...
export const createFunctionService = async (body: CreateFunctionType) => {
  ...
  try {
    ...
    const createdFunction = await pool.query(`INSERT INTO functions(runtime, fn_name, fn_zip_file, owner, response_url) VALUES($1, $2, $3, $4, $5) RETURNING *`, [runtime, fn_name, fn_zip_file, owner, response_url]);
...
export const getFunctionByIdService = async (functionId: string) => {
  try {
    ...
    const functionData = await pool.query("SELECT * FROM functions WHERE id=$1::uuid", [functionId])
...
export const getAllFunctionsService = async (userId: string) => {
  try {
    const functionsData = await pool.query(`SELECT * FROM functions WHERE owner=$1::uuid`, [userId]);
...
export const deleteFunctionByIdService = async (functionId: string) => {
  try {
    const functionData = await pool.query(`DELETE FROM functions WHERE id=$1 RETURNING *`,[functionId])
    return functionData.rows[0]
